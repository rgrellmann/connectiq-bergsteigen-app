...
class SunCalc {
    ...
    hidden const PI   = Math.PI,
        RAD  = Math.PI / 180.0,
        PI2  = Math.PI * 2.0,
        SECONDS_PER_DAY = Time.Gregorian.SECONDS_PER_DAY,
        J1970 = 2440588,
        J2000 = 2451545,
        J0 = 0.0009;
    ...
    function fromJulian(j) {
        return Time.Gregorian.info(new Time.Moment((j + 0.5 - J1970) * SECONDS_PER_DAY), Time.FORMAT_SHORT);
    }
    ...
    function calculate(moment, lat, lng, what) {
        var d = moment.value().toDouble() / SECONDS_PER_DAY - 0.5 + J1970 - J2000;
